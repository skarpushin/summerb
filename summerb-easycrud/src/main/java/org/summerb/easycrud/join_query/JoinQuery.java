...
public interface JoinQuery<TId, TRow extends HasId<TId>> {
  ...
  /**
   * @return Only queries which participate in the join sequence. Queries added via (NOT) EXISTS are
   *     not listed here.
   */
  List<Query<?, ?>> getJoinedQueries();

  /**
   * @return All participating queries, regardless of their placement
   */
