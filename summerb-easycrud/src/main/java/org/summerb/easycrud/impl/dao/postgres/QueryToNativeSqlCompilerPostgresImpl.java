...
import org.summerb.easycrud.impl.dao.mysql.ConditionConverter;
      ...
      new ConditionConverter<StringEqRestriction>() {
        ...
        public String convert(
          ...
          return String.format(
              "lower(%s) %s :%s", underscoredFieldName, r.isNegative() ? " != " : " = ", pn);
        }
      };
  ...
  protected ConditionConverter<StringOneOfRestriction> stringOneOfRestriction =
      new ConditionConverter<StringOneOfRestriction>() {
        ...
        public String convert(
          ...
          String pn = pname(paramIdx);
          params.addValue(
              pn, r.getValues().stream().map(x -> x.toLowerCase()).collect(Collectors.toList()));
          return String.format(
              "lower(%s) %s (:%s)", underscoredFieldName, r.isNegative() ? " NOT IN " : " IN ", pn);
        }
      };
}
