...
public interface AuthTokenService {
  ...
  /**
   * Verify token validity and if valid updated it with new values for tokenValue and lastUpdated.
   *
   * <p>This is a convenient all-in-one method. You don't have to use it if you want to implement
   * custom token validation checks, don't forget to use {@link #updateToken(String, long, String)}
   * to mitigate the "stolen token database" situation
   *
   * @param userUuid user which this auth token related to
   * @param authTokenUuid auth token unique id
   * @param tokenValue auth token value (AuthTokenService will generate new value for this field
   *     after successful validation)
   * @return new Auth token or null if token is not valid
   * @throws UserNotFoundException if user not found
   */
  AuthToken isAuthTokenValid(String userUuid, String authTokenUuid, String tokenValue)
   ...
   * @param newTokenValue new value for tokenValue field. It MUST NOT be equal to previous value
   * @throws AuthTokenNotFoundException if auth token is not found
   * @throws ValidationException if newTokenValue = current tokenValue for that token
   */
