...
public class PermissionServiceImpl implements PermissionService {
  ...
  public void grantPermission(
      String optionalDomainName, String userUuid, String optionalSubjectId, String permissionKey) {
    ...
    try {
      ...
      // it's ok
      log.debug(
          "Duplicate key exception sealed. Looks like same permission is already granted.", dke);
    } catch (Throwable t) {
      String msg =
          String.format(
              "Failed to grant permission '%s' to user '%s' on subject '%s' in domain '%s'",
  ...
  public void revokePermission(
    ...
    try {
      ...
      String msg =
          String.format(
              "Failed to revoke permission '%s' from user '%s' for subject '%s' in domain '%s'",
              permissionKey, userUuid, subjectId, domainName);
      throw new UserServiceUnexpectedException(msg, t);
    }
  }
  ...
  public void revokeUserPermissions(String optionalDomainName, String userUuid) {
    ...
    try {
      ...
      String msg =
          String.format(
              "Failed to revoke user '%s' permissions in domain '%s'", userUuid, domainName);
      throw new UserServiceUnexpectedException(msg, t);
    }
  }
  ...
  public void revokeAllPermissionsForSubject(String optionalDomainName, String optionalSubjectId) {
    ...
    try {
      ...
      String msg =
          String.format(
              "Failed to clear subject '%s' permissions in domain '%s'", subjectId, domainName);
      throw new UserServiceUnexpectedException(msg, t);
    }
  }

  @Override
  public List<String> findUserPermissionsForSubject(
      String optionalDomainName, String userUuid, String optionalSubjectId) {
    Preconditions.checkArgument(StringUtils.hasText(userUuid));
    String domainName = getOptionalParamValue(optionalDomainName);
    String subjectId = getOptionalParamValue(optionalSubjectId);
    ...
    try {
      ...
      String msg =
          String.format(
              "Failed to get user '%s' permissions for subject '%s' in domain '%s'",
  ...
  public boolean hasPermission(
    ...
    List<String> subjectPermissions =
        findUserPermissionsForSubject(optionalDomainName, userUuid, optionalSubjectId);
    return subjectPermissions.contains(permissionKey);
  }

  @Override
  public List<String> findSubjectsUserHasPermissionsFor(
      String optionalDomainName, String userUuid, String optionalRequiredPermission) {
    Preconditions.checkArgument(StringUtils.hasText(userUuid));
    String domainName = getOptionalParamValue(optionalDomainName);
    ...
    try {
      if (!StringUtils.hasText(optionalRequiredPermission)) {
        ...
        return permissionDao.getSubjectsUserHasPermissionForFiltered(
            domainName, userUuid, optionalRequiredPermission);
      }
    } catch (Throwable t) {
      String msg =
          String.format(
  ...
  @Override
  public Map<String, List<String>> findUsersAndTheirPermissionsForSubject(
    ...
    try {
      ...
      String msg =
          String.format(
              "Failed to get users and their permissions for subject '%s' in domain '%s'",
