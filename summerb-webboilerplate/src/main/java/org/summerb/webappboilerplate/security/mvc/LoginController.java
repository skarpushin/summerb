...
public class LoginController extends ControllerBase {
  ...
  @RequestMapping(
      ...
      value = SecurityActionsUrlsProviderDefaultImpl.LOGIN_FORM)
  ...
  @RequestMapping(
      method = RequestMethod.GET,
      value = SecurityActionsUrlsProviderDefaultImpl.LOGIN_FAILED)
  public String handleLoginFailed(Model model, HttpServletRequest request) {
    ...
    if (lastException != null) {
      ...
      model.addAttribute(
          "lastExceptionMessage", exceptionTranslatorSimplified.buildUserMessage(lastException));
    ...
    ValidationException validationErrors =
        ExceptionUtils.findExceptionOfType(lastException, ValidationException.class);
    if (validationErrors != null) {
      for (ValidationError error : validationErrors.getErrors()) {
        model.addAttribute(
            "ve_" + error.getPropertyName(), msg(error.getMessageCode(), error.getMessageArgs()));
      }
    }
  ...
  @RequestMapping(
      method = RequestMethod.GET,
      value = SecurityActionsUrlsProviderDefaultImpl.INVALID_SESSION)
  public ModelAndView handleInvalidSession(
  ...
  @RequestMapping(
      method = RequestMethod.GET,
      value = SecurityActionsUrlsProviderDefaultImpl.REGISTER)
  ...
  public String processRegisterForm(
      @ModelAttribute(ATTR_REGISTRATION) Registration registration,
      Model model,
      HttpServletRequest request) {
    if (!isAutoTestMode) {
    ...
    try {
      ...
      if (isDevMode) {
        ...
        model.addAttribute(
            UserAccountChangeHadlersDefaultImpl.ATTR_ACTIVATION_LINK, activationAbsoluteLink);
      }
    } catch (ValidationException fve) {
      model.addAttribute(
          ControllerBase.ATTR_VALIDATION_ERRORS, new ValidationErrorsVm(fve.getErrors()));
    }
    return views.registerForm();
  }
  ...
  @RequestMapping(
      method = RequestMethod.GET,
      value = SecurityActionsUrlsProviderDefaultImpl.ACTIVATE)
  public String getRegistrationActivationForm(
      Model model,
      HttpServletRequest request,
      @RequestParam(value = SecurityActionsUrlsProviderDefaultImpl.PARAM_ACTIVATION_UUID)
  ...
  @RequestMapping(
      method = RequestMethod.GET,
      value = SecurityActionsUrlsProviderDefaultImpl.REQUEST_RESET)
  ...
  public String processPasswordResetRequestForm(
      @ModelAttribute(ATTR_PASSWORD_RESET_REQUEST) Registration registration,
      Model model,
      HttpServletRequest request) {
    if (!isAutoTestMode) {
      CaptchaController.assertCaptchaTokenValid(
          "request-reset", registration.getCaptcha(), request);
    }

    try {
      String passwordResetToken =
          usersServiceFacade.getNewPasswordResetToken(registration.getEmail());

      // Generate registration link
      String passwordResetAbsoluteLink =
          absoluteUrlBuilder.buildExternalUrl(
              securityActionsUrlsProvider.buildPasswordResetPath(
                  registration.getEmail(), passwordResetToken));

      model.addAttribute(ATTR_FORM_ACCEPTED, true);
      if (isDevMode) {
        model.addAttribute(
            UserAccountChangeHadlersDefaultImpl.ATTR_PASSWORD_RESET_LINK,
  ...
  @RequestMapping(
      method = RequestMethod.GET,
      value = SecurityActionsUrlsProviderDefaultImpl.RESET_PASSWORD)
  public String getPasswordResetForm(
      @PathVariable(ATTR_PASSWORD_RESET_TOKEN) String passwordResetToken,
      @RequestParam(User.FN_EMAIL) String email,
      Model model,
      HttpServletRequest request)
  ...
  @RequestMapping(
      method = RequestMethod.GET,
      value = SecurityActionsUrlsProviderDefaultImpl.CHANGE_PASSWORD)
  ...
  @RequestMapping(
      method = RequestMethod.POST,
      value = SecurityActionsUrlsProviderDefaultImpl.CHANGE_PASSWORD)
  public String processPasswordChangeForm(
      @ModelAttribute(ATTR_PASSWORD_CHANGE) PasswordChange passwordChange,
      Model model,
      HttpServletRequest request)
      throws UserNotFoundException {
  ...
  @Autowired(required = false)
  public void setSecurityActionsUrlsProvider(
      SecurityActionsUrlsProvider securityActionsUrlsProvider) {
  ...
  @Autowired
  public void setExceptionTranslatorSimplified(
      ExceptionTranslatorSimplified exceptionTranslatorSimplified) {
