 ...
 * Default impl.
 *
 * <p>It's defined as a separate class (separate from {@link LoginController} to have it in root
 * context, rather in servlet context
 *
 * <p>See {@link SecurityActionsUrlsProvider}
 *
 * @author sergeyk
 */
public class SecurityActionsUrlsProviderDefaultImpl implements SecurityActionsUrlsProvider {
  ...
  public String buildPasswordResetPath(String username, String passwordResetToken) {
    return RESET_PASSWORD.replace("{passwordResetToken}", passwordResetToken)
        + "?"
        + User.FN_EMAIL
        + "="
        + username;
