 ...
 *
 * <p>It's designed to be added to {@link Model} and used in JSP
 *
...
public class ValidationErrorsVm {
  ...
  private Map<String, String> hideCssClass =
      new DummyMapImpl<String, String>() {
        @Override
        public String get(Object key) {
          // NOTE: This "error" literal is actually a css class name. This way
  ...
  /**
   * @return Map that maps field token to error msg. Message will be null if there is no error for
   *     that field.
   */
  public Map<String, String> getMsg() {
    if (errorsMap == null) {
      ...
      for (ValidationError validationError : validationErrors) {
        try {
          String message =
              I18nUtils.buildMessage(
                  validationError, CurrentRequestUtils.getWac(), LocaleContextHolder.getLocale());
  ...
  /**
   * @return Map that maps field token to error result. Error result is an empty string if there is
   *     no error and "error" if there is an error for that field (field is a key for this map)
   */
