...
public abstract class ControllerBase implements ApplicationContextAware, InitializingBean {
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    if (exceptionHandlerStrategy == null) {
      ...
      // projects
      ControllerExceptionHandlerStrategyLegacyImpl handler =
          new ControllerExceptionHandlerStrategyLegacyImpl(applicationContext);
  ...
  public static void addMessage(
      Map<String, Object> modelMap, PageMessage pageMessage, String messageBundleName) {
  ...
  /**
   * Alternative way to handle exceptions when handling requests. Instead of using generic error
   * handler we might want to show the same page, but with error content; which is being set as
   * model attribute {@link #ATTR_EXCEPTION}
   */
  protected void mapExceptionsToModelAttr(Model model, Callable<?> callable) throws Exception {
    try {
      ...
      if (ExceptionUtils.findExceptionOfType(t, HasMessageCode.class) != null) {
        String userMessage =
            exceptionTranslator.buildUserMessage(t, LocaleContextHolder.getLocale());
  ...
  public void setExceptionHandlerStrategy(
      ControllerExceptionHandlerStrategy exceptionHandlerStrategy) {
  ...
  @Autowired(required = false)
  public void setSecurityContextResolver(
      SecurityContextResolver<? extends User> securityContextResolver) {
