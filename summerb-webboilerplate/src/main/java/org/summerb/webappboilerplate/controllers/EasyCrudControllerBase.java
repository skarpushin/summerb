 ...
 * @deprecated use {@link EasyCrudRestControllerBase} instead, it's better than this. This
 *     controller happened to be very clumsy. Will remove it in the near future. Most of it's
 *     methods are REST API, but naming is discouraged and 3 regular MVC actions brings entropy
 */
public class EasyCrudControllerBase<
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    Preconditions.checkState(
        StringUtils.hasText(viewNameForList), "viewNameForList Expected to be init by subclass");
    Preconditions.checkState(
        StringUtils.hasText(viewNameForOne), "viewNameForOne Expected to be init by subclass");
    Preconditions.checkState(
        StringUtils.hasText(viewNameForJs), "viewNameForJs Expected to be init by subclass");
  }
  ...
  protected Map<String, Boolean> getPermissionsMapForCurrentUser() {
    ...
    if (service instanceof HasEasyCrudTableAuthStrategy) {
      ...
      throw new IllegalStateException(
          "Not supported service type, can't resolve permissions for: " + service);
    }

    return ret;
  }

  private void resolveEasyCrudTableAuthStrategyPermissions(
      EasyCrudTableAuthStrategy auth, Map<String, Boolean> ret) {
    try {
  ...
  public @ResponseBody Map<String, ? extends Object> ajaxCreate(
      @RequestBody TDto dto, HttpServletResponse response) throws Exception {
  ...
  public @ResponseBody Map<String, ? extends Object> ajaxDelete(
      @PathVariable("entityId") TId id, HttpServletResponse response) throws Exception {
    service.deleteById(id);
    return Collections.singletonMap(ATTR_ID, id);
  }

  @RequestMapping(method = RequestMethod.GET, value = "{entityId}")
  public ModelAndView getOne(@PathVariable("entityId") TId id, HttpServletResponse response)
      throws Exception {
  ...
  public @ResponseBody Map<String, ? extends Object> ajaxUpdate(
      @RequestBody TDto dto, HttpServletResponse response) throws Exception {
  ...
  public void setFilteringParamsToQueryConverter(
      FilteringParamsToQueryConverter filteringParamsToQueryConverter) {
