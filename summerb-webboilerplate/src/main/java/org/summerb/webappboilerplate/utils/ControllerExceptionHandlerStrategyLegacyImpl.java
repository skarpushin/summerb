 ...
 *
 * @deprecated don't use this impl. It's cumbersome and confusing MVC and REST API approaches. It
 *     handles REST API exceptions as well while it should be handled by {@link
...
public class ControllerExceptionHandlerStrategyLegacyImpl
  ...
  @Override
  public ModelAndView handleUnexpectedControllerException(
      Throwable ex, HttpServletRequest req, HttpServletResponse res) {
    boolean isAcceptJson =
        req.getHeader("Accept") != null && req.getHeader("Accept").startsWith("application/json");
    ...
    Throwable contained =
        ex instanceof NestedServletException && ex.getCause() != null ? ex.getCause() : ex;
  ...
  protected ModelAndView buildHtmlError(Throwable ex) {
    if (securityContextResolver != null
        && (ex instanceof AccessDeniedException
            && !securityContextResolver.hasRole(Roles.ROLE_USER))) {
      throw new IllegalArgumentException(
          "Exception will not be handled by default exception handler: " + ex);
    }
    ...
    ret.getModel()
        .put(
            ControllerBase.ATTR_EXCEPTION_STACKTRACE,
            ExceptionUtils.getThrowableStackTraceAsString(ex));
    return ret;
  }

  /**
   * This peace of crap needs to be removed. Because in case of JSON it's rest API, there is no
   * place for {@link ModelAndView}. Response should be pure JSON content.
   *
   * <p>So instead of implementing it here it's better to just re-throw exception and let {@link
   * RestExceptionTranslator} handle it and gracefully convert it into json description of error
   * happened
   */
  protected ModelAndView buildJsonError(
      Throwable ex, HttpServletRequest req, HttpServletResponse res) {
    String msg = exceptionTranslator.buildUserMessage(ex, LocaleContextHolder.getLocale());
    ...
    boolean translateAuthExc =
        Boolean.TRUE.equals(
            Boolean.valueOf(
                req.getHeader(RestExceptionTranslator.X_TRANSLATE_AUTHORIZATION_ERRORS)));
    if ((nae = ExceptionUtils.findExceptionOfType(ex, NotAuthorizedException.class)) != null) {
      ...
      res.setStatus(
          isAnonymous() ? HttpServletResponse.SC_UNAUTHORIZED : HttpServletResponse.SC_FORBIDDEN);
      if (translateAuthExc) {
      ...
      if (translateAuthExc) {
        ...
        respondWithJson(
            new NotAuthorizedResult(getCurrentUser(), SecurityMessageCodes.ACCESS_DENIED), res);
        return null;
      }
