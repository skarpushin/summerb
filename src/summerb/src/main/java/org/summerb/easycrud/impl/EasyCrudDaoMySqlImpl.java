 ...
 *
 */
public class EasyCrudDaoMySqlImpl<TId, TDto extends HasId<TId>> extends DaoBase implements EasyCrudDao<TId, TDto>,
		InitializingBean {
	...
	public PaginatedList<TDto> query(PagerParams pagerParams, Query optionalQuery, OrderBy... orderBy) {
		MapSqlParameterSource params = new MapSqlParameterSource();
		String whereClause = optionalQuery == null ? "" : "WHERE "
		...
		List<TDto> list = jdbc.query(sqlFindByCustomQuery + whereClause + buildOrderBySubclause(orderBy)
				+ sqlPartPaginator, params, rowMapper);
		@SuppressWarnings("deprecation")
		int totalResultsCount = jdbc.queryForInt(sqlFindByCustomQueryForCount + whereClause, params);
