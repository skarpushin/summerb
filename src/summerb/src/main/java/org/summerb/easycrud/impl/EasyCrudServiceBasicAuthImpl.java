...
public class EasyCrudServiceBasicAuthImpl<TId, TDto extends HasId<TId>> implements EasyCrudService<TId, TDto>,
		InitializingBean {
	...
	private EasyCrudBasicAuthStrategy<TDto> basicAuthStrategy;
	...
	public void afterPropertiesSet() throws Exception {
		...
		Preconditions.checkState(basicAuthStrategy != null, "basicAuthStrategy is required");
	...
	public TDto create(TDto dto) throws FieldValidationException, NotAuthorizedException {
		try {
			...
			basicAuthStrategy.assertAuthorizedToCreate(dto);
	...
	public TDto update(TDto newVersion) throws FieldValidationException, NotAuthorizedException,
		...
		try {
			...
			basicAuthStrategy.assertAuthorizedToUpdate(currentVersion, newVersion);
	...
	public void deleteById(TId id) throws NotAuthorizedException, EntityNotFoundException {
		try {
			...
			basicAuthStrategy.assertAuthorizedToDelete(existing);
	...
	public int deleteByQuery(Query query) throws NotAuthorizedException {
		try {
			...
			PaginatedList<TDto> toDelete = query(new PagerParams(0, Long.MAX_VALUE), query);
			List<TDto> deleted = new ArrayList<TDto>();
			for (TDto dto : toDelete.getItems()) {
				basicAuthStrategy.assertAuthorizedToDelete(dto);
	...
	public TDto findById(TId id) throws NotAuthorizedException {
		try {
			...
			if (ret != null) {
				basicAuthStrategy.assertAuthorizedToRead(ret);
			}
			return ret;
		} catch (Throwable t) {
	...
	public PaginatedList<TDto> query(PagerParams pagerParams, Query optionalQuery, OrderBy... orderBy)
		...
		try {
			...
			for (TDto dto : ret.getItems()) {
				basicAuthStrategy.assertAuthorizedToRead(dto);
			}
			return ret;
		} catch (Throwable t) {
			throw exceptionStrategy.handleExceptionAtFind(t);
	...
	public EasyCrudBasicAuthStrategy<TDto> getBasicAuthStrategy() {
		return basicAuthStrategy;
	}

	public void setBasicAuthStrategy(EasyCrudBasicAuthStrategy<TDto> basicAuthStrategy) {
