...
public class QueryToNativeSqlCompilerMySqlImpl implements QueryToNativeSqlCompiler {
	...
	protected Map<Class<? extends Restriction<?>>, ConditionConverter<? extends Restriction<?>>> converters = new HashMap<>();
	...
	protected void buildWhereClauseAndPopulateParams(Query query, MapSqlParameterSource params, AtomicInteger paramIdx,
			StringBuilder sb) {
		sb.append("(");
	...
	private ConditionConverter<BooleanEqRestriction> booleanEqRestriction = new ConditionConverter<BooleanEqRestriction>() {
		@Override
		public String convert(BooleanEqRestriction r, MapSqlParameterSource params, AtomicInteger paramIdx,
	...
	private ConditionConverter<IsNullRestriction> isNullRestriction = new ConditionConverter<IsNullRestriction>() {
		@Override
		public String convert(IsNullRestriction r, MapSqlParameterSource params, AtomicInteger paramIdx,
				String underscoredFieldName) {
			return underscoredFieldName + (r.isNegative() ? " IS NOT NULL" : " IS NULL");
		}
	};

	private ConditionConverter<NumberBetweenRestriction> numberBetweenRestriction = new ConditionConverter<NumberBetweenRestriction>() {
		@Override
		public String convert(NumberBetweenRestriction r, MapSqlParameterSource params, AtomicInteger paramIdx,
				String underscoredFieldName) {
			...
			if (!r.isNegative()) {
				...
				return String.format("(%s < :%s OR :%s < %s)", underscoredFieldName, pnLower, pnUpper,
						underscoredFieldName);
			}
		}
	};

	private ConditionConverter<StringBetweenRestriction> stringBetweenRestriction = new ConditionConverter<StringBetweenRestriction>() {
		@Override
		public String convert(StringBetweenRestriction r, MapSqlParameterSource params, AtomicInteger paramIdx,
				String underscoredFieldName) {
			String pnLower = pname(paramIdx);
			String pnUpper = pname(paramIdx);
			params.addValue(pnLower, r.getLowerBound());
	...
	private ConditionConverter<StringLengthBetweenRestriction> stringLengthBetweenRestriction = new ConditionConverter<StringLengthBetweenRestriction>() {
		@Override
		public String convert(StringLengthBetweenRestriction r, MapSqlParameterSource params, AtomicInteger paramIdx,
				String underscoredFieldName) {
			String pnLower = pname(paramIdx);
			String pnUpper = pname(paramIdx);
			params.addValue(pnLower, r.getLowerBound());
			params.addValue(pnUpper, r.getUpperBound());
			return String.format("(CHAR_LENGTH(%s) BETWEEN :%s AND :%s)", underscoredFieldName, pnLower, pnUpper);
		}
	};

	private ConditionConverter<NumberEqRestriction> numberEqRestriction = new ConditionConverter<NumberEqRestriction>() {
		@Override
		public String convert(NumberEqRestriction r, MapSqlParameterSource params, AtomicInteger paramIdx,
				String underscoredFieldName) {
			String pn = pname(paramIdx);
	...
	private ConditionConverter<NumberGreaterOrEqualRestriction> numberGreaterOrEqualRestriction = new ConditionConverter<NumberGreaterOrEqualRestriction>() {
		@Override
		public String convert(NumberGreaterOrEqualRestriction r, MapSqlParameterSource params, AtomicInteger paramIdx,
				String underscoredFieldName) {
			String pn = pname(paramIdx);
			params.addValue(pn, r.getValue());
			return underscoredFieldName + (r.isNegative() ? " < :" : " >= :") + pn;
	...
	private ConditionConverter<NumberOneOfRestriction> numberOneOfRestriction = new ConditionConverter<NumberOneOfRestriction>() {
		@Override
		public String convert(NumberOneOfRestriction r, MapSqlParameterSource params, AtomicInteger paramIdx,
				String underscoredFieldName) {
			String pn = pname(paramIdx);
