...
public class AttachmentDaoImpl extends EasyCrudDaoMySqlImpl<Long, Attachment>
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    sqlUpdateFileContents =
        String.format("UPDATE %s SET contents = :contents WHERE id = :id", getTableName());
  }
  ...
  private RowMapper<InputStream> rowMapperFileContents =
  ...
  public void putContentInputStream(long id, InputStream contents) {
    ...
    if (affected != 1) {
      throw new RuntimeException(
          "Failed to update attachment record with contents, "
              + affected
              + " records affected instead of 1");
    }
  }
  ...
  public InputStream getContentInputStream(long id) {
    ...
    List<InputStream> results =
        jdbc.query(sqlGetFileContentsByUuid, paramMap, rowMapperFileContents);
