...
/**
 * This bean will listen on an event bus (preferably tx-based event bus, so after transaction is
 * completed it will remove article's attachments if it was deleted)
 *
 * @author sergeyk
 */
public class AttachmentFilesRemover implements InitializingBean {
  ...
  /**
   * Provide an {@link EventBus} which will be used to transmit {@link EntityChangedEvent} upon
   * transaction completion.
   *
   * @param eventBus It could be any event bus, but it's batter to have tx-bound one so that files
   *     are deleted only if transaction committed
   */
  @Required
