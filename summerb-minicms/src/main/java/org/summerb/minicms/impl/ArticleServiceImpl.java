...
public class ArticleServiceImpl extends EasyCrudServicePluggableImpl<Long, Article, ArticleDao>
  ...
  public Article findArticleByKeyAndLocale(String key, Locale locale)
    ...
    Article ret =
        findOneByQuery(Query.n().eq(Article.FN_KEY, key).eq(Article.FN_LANG, locale.getLanguage()));
    if (ret == null && !fallbackToLocale.getLanguage().equalsIgnoreCase(locale.getLanguage())) {
      ret =
          findOneByQuery(
              Query.n()
                  .eq(Article.FN_KEY, key)
                  .eq(Article.FN_LANG, fallbackToLocale.getLanguage()));
    }
    return ret;
  }
  ...
  public Map<Locale, Article> findArticleLocalizations(String articleKey) {
    try {
      PaginatedList<Article> articleOptions =
          find(PagerParams.ALL, Query.n().eq(Article.FN_KEY, articleKey));
  ...
  public List<Article> findByGroup(String group, Locale locale) throws NotAuthorizedException {
    ...
    return find(
            PagerParams.ALL,
            Query.n().eq(Article.FN_GROUP, group).eq(Article.FN_LANG, locale.getLanguage()))
        .getItems();
  }
