 ...
 * value class.
 *
 * <p>IMPORTANT: In order to significantly decrease potential vulnerability of using {@link
 * Class#forName(String)} underlying DTOs are required to implement {@link DtoBase} interface.
 *
 * @author sergeyk
 */
...
public class EntityChangedEventAdapter
  ...
  @Override
  public EntityChangedEvent deserialize(
  ...
  protected <T extends DtoBase> Class<T> resolveParametersClass(JsonObject jsonObject) {
    ...
    if (prim == null) {
      throw new IllegalArgumentException(
          "JSON Parse error: didn't find classname field named '"
              + CLASSNAME
              + "' in object: "
              + jsonObject);
    }
