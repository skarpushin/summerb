...
/**
 * Validation error which acts as a container for collection of objects which were validated
 *
 * @author sergey.karpushin
 */
public class AggregatedObjectsValidationErrors extends ValidationError
    implements HasValidationErrors {
  ...
  /**
   * @param fieldToken name of the field which references custom object (not just simple scalar
   *     variable) which was validated and contains errors. In case if this DTO is used to represent
   *     object in collection, this field might be used to identify collection item instance (it
   *     might be item idx, guid or any other thing suitable for end-application)
   * @param aggregatedObjectValidationErrors list of containers, which represent validation errors
   *     for specific items from collection
   */
  public AggregatedObjectsValidationErrors(
      String fieldToken, List<AggregatedObjectValidationErrors> aggregatedObjectValidationErrors) {
    super("validation.aggregatedObjectsError", fieldToken);
    Preconditions.checkArgument(
        aggregatedObjectValidationErrors != null,
