...
public class DbSchemaVersionResolverImpl implements DbSchemaVersionResolver {
  ...
  public DbSchemaVersionResolverImpl(
    ...
    try {
      ...
      if (versionTableDbDialect.isTableMissingException(exc)) {
        ...
        throw new RuntimeException(
            "Failed to verify if database contains table for tracking db versions", exc);
      }
    }
    this.jdbcInsert =
        new SimpleJdbcInsert(dataSource).withTableName(versionTableDbDialect.getTableName());
    ...
    try {
      ...
      Integer ret =
          jdbcTemplate.queryForObject(
              versionTableDbDialect.getCurrentDbVersionQuery(), Integer.class);
  ...
  public void increaseVersionTo(int version) {
    try {
      ...
      Preconditions.checkArgument(
          getCurrentDbVersion() < version,
          "New version %s must be higher than current %s",
          version,
          getCurrentDbVersion());

      BeanPropertySqlParameterSource params =
          new BeanPropertySqlParameterSource(new DbVersion(version));
