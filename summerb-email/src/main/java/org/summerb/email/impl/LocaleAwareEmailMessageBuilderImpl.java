...
/**
 * This impl doesn't actually implement email building procedure, but it knows how to delegate to
 * approriate builder according to current locale.
 *
 * <p>Make use of Spring's {@link LocaleContextHolder}
 *
 * @author skarpushin
 */
public class LocaleAwareEmailMessageBuilderImpl implements EmailMessageBuilder {
  ...
  public EmailMessage buildEmail(
      String fromAddress, String toAddress, EmailTemplateParams emailTemplateParams) {
    Preconditions.checkState(
        !CollectionUtils.isEmpty(localeSpecificBuilders),
  ...
  protected EmailMessageBuilder getBuilder() {
    ...
    log.warn(
        String.format(
            "Was unable to find appropriate email builder for locale '%s', will peek the first one",
